...
const getNavigationItems = (isAdmin: boolean, isSignedIn: boolean) => {
    if (isAdmin) {
        return [
...
export function UniversalSidebar({ isAdmin = false }: UniversalSidebarProps) {
    ...
    const navigation = getNavigationItems(isAdmin, !!isSignedIn);
    const sidebarTitle = isAdmin ? 'Audio Admin' : 'Audio Player';
    const sidebarSubtitle = isAdmin ? 'Management Panel' : 'Music Streaming';
