...
import { AdminTrackSearch } from '@/components/admin/search/admin-track-search';
import type { TrackWithDetails } from '@/app/admin/tracks/page';

type AdminTracksClientProps = {
    initialTracks: TrackWithDetails[];
};

export function AdminTracksClient({ initialTracks }: AdminTracksClientProps) {
    const [displayedTracks, setDisplayedTracks] = useState(initialTracks);
    const [isSearching, setIsSearching] = useState(false);

    const handleSearchResults = useCallback((results: TrackWithDetails[], searching: boolean) => {
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                        {isSearching ? `${displayedTracks.length} search results` : `${initialTracks.length} total tracks`}
                    </p>
                </div>
            ...
            {isSearching && (
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                    ...
                    <p className="text-sm text-blue-700 dark:text-blue-300">
                        Found {displayedTracks.length} tracks matching your search
                    </p>
                </div>
            )}
            ...
            {displayedTracks.length === 0 && (
                <div className="text-center py-12 bg-gradient-to-br from-white via-gray-50/50 to-blue-50/40 dark:bg-gray-800 rounded-xl border-2 border-gray-200/80 dark:border-gray-700 shadow-lg shadow-gray-200/40 dark:shadow-none theme-transition">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 mb-4">
                        {isSearching ? 'Try adjusting your search terms.' : 'Get started by creating your first track.'}
                    </p>
                    {!isSearching && (
                        <Link href="/admin/tracks/new" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 transition-all duration-200 text-sm font-medium btn-transition shadow-sm hover:shadow-md">
                            <Plus className="h-4 w-4" />
