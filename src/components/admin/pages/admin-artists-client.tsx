...
import { AdminArtistSearch } from '@/components/admin/search/admin-artist-search';
...
export function AdminArtistsClient({ initialArtists }: AdminArtistsClientProps) {
    const [displayedArtists, setDisplayedArtists] = useState(initialArtists);
    const [isSearching, setIsSearching] = useState(false);

    const handleSearchResults = useCallback((results: Artist[], searching: boolean) => {
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                        {isSearching ? `${displayedArtists.length} search results` : `${initialArtists.length} total artists`}
                    </p>
                </div>
            ...
            {isSearching && (
                <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4">
                    ...
                    <p className="text-sm text-purple-700 dark:text-purple-300">
                        Found {displayedArtists.length} artists matching your search
                    </p>
                </div>
            )}
            ...
            {displayedArtists.length === 0 && (
                <div className="text-center py-12 bg-gradient-to-br from-white via-gray-50/50 to-purple-50/40 dark:bg-gray-800 rounded-xl border-2 border-gray-200/80 dark:border-gray-700 shadow-lg shadow-gray-200/40 dark:shadow-none theme-transition">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                        {isSearching ? 'No artists found' : 'No artists found'}
                    </h3>
                    <p className="text-gray-600 dark:text-gray-400 mb-4">
                        {isSearching ? 'Try adjusting your search terms.' : 'Get started by creating your first artist.'}
                    </p>
                    {!isSearching && (
                        <Link href="/admin/artists/new" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 transition-all duration-200 text-sm font-medium btn-transition shadow-sm hover:shadow-md">
                            <Plus className="h-4 w-4" />
