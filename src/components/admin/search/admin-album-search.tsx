...
import { searchAdminAlbums } from '@/actions/admin-search';
...
type AdminAlbumSearchProps = {
    allAlbums: AlbumWithArtists[];
    onSearchResults: (results: AlbumWithArtists[], isSearching: boolean) => void;
};

export function AdminAlbumSearch({ allAlbums, onSearchResults }: AdminAlbumSearchProps) {
    ...
    useEffect(() => {
        const timeoutId = setTimeout(async () => {
            if (searchQuery.trim()) {
                setIsSearching(true);
                try {
                    const searchResults = await searchAdminAlbums(searchQuery, 50);
