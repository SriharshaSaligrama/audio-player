...
import { searchArtistsWithFollowStatus } from '@/actions/artists';
import type { ArtistWithFollowStatus } from '@/actions/artists';

type ArtistSearchProps = {
    allArtists: ArtistWithFollowStatus[];
    onSearchResults: (results: ArtistWithFollowStatus[], isSearching: boolean) => void;
};

export function ArtistSearch({ allArtists, onSearchResults }: ArtistSearchProps) {
    ...
    useEffect(() => {
        const timeoutId = setTimeout(async () => {
            if (searchQuery.trim()) {
                setIsSearching(true);
                try {
                    const searchResults = await searchArtistsWithFollowStatus(searchQuery, 20);
